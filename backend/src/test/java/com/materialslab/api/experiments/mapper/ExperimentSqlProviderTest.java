...
class ExperimentSqlProviderTest {
    ...
    void 应按组织范围筛选并支持项目状态和关键字条件() {
        ...
        assertThat(sql).contains("e.organization_id = #{organizationId}", "#{readAll} = TRUE", "project_member", "e.project_id = #{projectId}", "e.status = #{status}", "e.name ILIKE");
    }
    ...
    void 总数查询应复用列表筛选条件() {
        ...
        assertThat(sql).contains("SELECT COUNT(*)", "e.organization_id = #{organizationId}", "#{readAll} = TRUE", "project_member", "e.project_id = #{projectId}", "e.status = #{status}", "e.name ILIKE");
    }
}
