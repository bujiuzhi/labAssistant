...
public interface ProjectDocumentMapper {
    ...
    /** 查询指定项目中的文档元数据。 */
    @Select("""
            SELECT d.id, d.organization_id, d.project_id, d.category, d.name, d.version_label, d.file,
    ...
    /** 读取历史数据库正文；新上传文档正文存放在对象存储。 */
    @Select("SELECT content FROM project_document_content WHERE document_id = #{documentId}")
    DocumentContentRow findLegacyContent(@Param("documentId") UUID documentId);
