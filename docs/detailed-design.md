...
创建时的 `member_ids` 和 PATCH 中显式给出的 `member_ids` 会在同一事务中同步为普通 `researcher` 成员，
只接受当前组织处于启用状态的账号。PATCH 未提供该字段时保留原成员，空数组则清空普通成员；
负责人不重复写入普通成员关系，创建者的 `manager` 关系不会被成员同步删除。
