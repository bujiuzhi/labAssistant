...
| 领域 | 已接通范围 | 限制 |
| --- | --- | --- |
| 身份与用户 | 本地密码登录、Session、CSRF、管理员签发一次性邀请码注册、普通用户创建/停用/密码重置/逻辑删除、自助改密；独立生产首次身份初始化 | 邀请注册仅创建预绑定角色的普通用户；用户管理和邀请码管理限超级管理员；删除前须交接活动资源负责人；初始化拒绝非空库 |
| 项目 | 查询、创建、基本字段/目标/里程碑/成员更新、关注、归档 | 人员组成仅维护普通实验员成员；项目负责人和创建者的管理关系独立保留；创建不具备后端幂等去重 |
...
当前实现的正式名称为“**多租户 RBAC + 项目成员数据范围**”（tenant-scoped RBAC with project-membership data scope）。它由以下四层共同构成，不能只用“角色权限”概括：

| 层次 | 当前术语 | 当前作用 |
| --- | --- | --- |
| 租户边界 | 组织（`organization_id`） | 账号、角色绑定、项目、实验、文件对象键和邀请码均归属一个组织；服务端按当前会话组织隔离。 |
| 动作授权 | 基于角色的访问控制（RBAC） | 权限码授予查看、创建、更新、归档、上传、状态迁移等动作能力。 |
| 数据范围 | 项目成员、实验负责人和实验参与人关系 | 在拥有动作权限后，进一步限制可读取或写入的具体项目、实验及其资源。 |
| 控制面特权 | 平台管理员标记 | 只用于组织目录和新组织开通，不授予其他组织业务数据访问权。 |

当前不提供通用 ACL、ABAC、ReBAC，也不支持同一账号跨组织成员身份或组织切换；这些是后续可演进能力，不是首版已交付功能。

组织角色决定功能权限，项目成员和实验参与关系进一步限制对象范围。

| 角色 | 功能权限 | 对象范围 |
| --- | --- | --- |
| 平台管理员 | 查看组织目录、开通新组织及首个组织管理员 | 归属内部平台组织，不读取或管理任何租户业务数据、成员或邀请码 |
...
当前组织用户选项还要求 `project.create` 权限，且仅返回当前组织的有效账户。权限码、关系角色及前后端限制详见详细设计。
生产首次 bootstrap 仅创建内部平台组织及平台管理员，初始业务组织数量为 0。平台管理员开通每个独立组织时，原子创建该组织首个超级管理员、三个内置角色及其权限；该组织管理员不继承平台权限。组织管理员可直接创建本组织普通用户，或签发绑定本组织角色和有效期的一次性邀请码；邀请码库仅保存 SHA-256 哈希，明文只在签发时返回一次。注册用户不能自行选择组织、角色或超级管理员身份。

当前一个登录账号只属于一个组织，用户名在全平台唯一；不支持同一账号跨组织成员身份、组织切换或平台管理员读取其他组织的业务数据。项目、实验、文件对象键、邀请码与普通用户管理均按当前会话组织隔离。
用户管理接口不能修改、重置密码或删除超级管理员，系统角色名称也不等于自动获得超级管理员标记。组织超级管理员可逻辑删除本组织普通用户：删除会撤销角色和未使用邀请码、匿名化账号并使已有 Session 在下一次请求立即失效，但保留历史项目、实验和审计外键；仍负责未归档项目或未完成实验的账号须先交接负责人。所有已登录用户均可验证旧密码后修改自己的密码，成功后当前 Session 立即失效并需重新登录。
